...
interface CustomPhoneInputProps {
  onChange: (phone: string) => void; // Prop to handle phone number change
}

const CustomPhoneInput : React.FC<CustomPhoneInputProps> = ({ onChange }) => {
  const [value, setValue] = useState("+1");
	const handleChange = (phone: string) => {
    setValue(phone);
    onChange(phone); // Call the onChange prop to update the parent state
  };
  return (
    <>
      {/* Global Style Overrides */}
      <style global jsx>{`
        .phone-input .phone-input-country-selector {
      ...
      <PhoneInput
        ...
        inputClassName="w-full h-11 !px-4 !border !border-gray-700 !rounded-lg placeholder-white/40 text-sm"
      />
    </>
  );
};
